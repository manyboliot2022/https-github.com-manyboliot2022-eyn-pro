...
const POS: React.FC<{ currentUser: UserProfile | null }> = ({ currentUser }) => {
  ...
  const findAndAdd = (code: string) => {
    const p = products.find(prod => (prod.barcode && prod.barcode === code) || prod.name.toLowerCase().includes(code.toLowerCase()));
    if (p) {
    ...
    <div className="h-full flex flex-col space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-white rounded-3xl pl-12 pr-6 py-5 text-sm font-black shadow-sm outline-none border border-slate-50 focus:border-slate-200 transition-all" 
            value={search} 
          ...
          cart.map(item => (
            <div key={item.id} className="bg-white p-5 rounded-[2.5rem] flex items-center gap-4 shadow-sm animate-fade border border-slate-50">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-slate-400 font-bold mt-0.5">{(item.sellPrice * item.quantity).toLocaleString()} FG</p>
              </div>
              ...
              <button onClick={() => setCart(cart.filter(i => i.id !== item.id))} className="p-2 text-red-200"><X className="w-5 h-5"/></button>
            </div>
          ))
        )}
      </div>

      {/* Panneau de Paiement NOIR / JAUNE style Photo */}
      <div className="bg-[#111827] p-8 rounded-[3rem] shadow-2xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
             ...
             <input 
               type="number" 
               placeholder="0" 
               className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-white text-sm font-black outline-none focus:border-yellow-400/50" 
               value={cashReceived} 
        ...
        <button 
          ...
          className="w-full bg-yellow-400 text-[#111827] py-6 rounded-[2rem] font-black uppercase text-xs tracking-[0.1em] flex items-center justify-center gap-3 active:scale-[0.98] transition-all disabled:opacity-20 shadow-xl shadow-yellow-400/10"
        >
      ...
      {isSuccess && (
        <div className="fixed inset-0 z-[500] bg-[#111827]/98 backdrop-blur-xl flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-xs rounded-[4rem] p-10 text-center space-y-8 animate-fade shadow-2xl">
            ...
            <div>
              <h2 className="text-xl font-black text-[#111827] uppercase tracking-tighter italic">VENTE RÉUSSIE</h2>
              <p className="text-[9px] font-bold text-slate-400 mt-2 uppercase tracking-widest leading-relaxed">Transaction enregistrée.<br/>Souhaitez-vous partager le reçu ?</p>
            </div>
            <div className="space-y-3">
              <button className="w-full bg-[#25D366] text-white py-5 rounded-3xl flex items-center justify-center gap-3 font-black uppercase text-[10px] tracking-widest shadow-lg shadow-green-500/20">
                <MessageCircle className="w-5 h-5" /> WhatsApp Client
              </button>
              <button onClick={() => {setCart([]); setIsSuccess(false); setCashReceived('');}} className="w-full bg-[#111827] text-white py-5 rounded-3xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">
                NOUVELLE VENTE
              </button>
            </div>
          </div>
